 ...
 */
Ext.define('App.utils.Environment',{
    ...
    constructor: function(){
        var savedDataHash = localStorage.getItem('environment'),
            ...
            callbacks = {},
            productionLocation = ['https://fbstat.mdc.gameloft.org/Tdw/', 'https://gat.gameloft.org/'],
            stagingKey = '7d8470139778fa48a82d60d0fb5fcd26fc69c18c';
        ...
        function getModuleName() {
            var href = location.href,
                moduleDirty = href.split('module=')[1],
                module = moduleDirty.indexOf('&') == -1 ? moduleDirty : moduleDirty.split('&')[0];
            return module;
        }
        /**
         * Check if the application using in development environment,
         * if yes return true, in other way return false
         *
         * @return {Boolean}
         */
        this.isDevelopment = function() {
            return !(location.href.indexOf('development=1') === -1);
        };
        /**
         * Check if the application using in YOY environment,
         * if yes return true, in other way return false
         *
         * @return {Boolean}
         */
        this.isYoy = function() {
            return !(location.href.indexOf('useYoy=1') === -1);
        };
        /**
         * Check if application necessary located on GAT server (scan be located on local or staging server)
         *
         * @return {Boolean}
         */
        this.isProductionServer = function() {
            var isProduction = false;
            for(var index in productionLocation){
                isProduction = location.href.indexOf(productionLocation[index]) === 0;
                if(isProduction){
                    break;
                }
            }
        ...
        this.getValueFromRequest = function(key) {
            var strSearch = location.search,
                str = strSearch.split("?"),
                strKeys = str[1] ? str[1].split("&") : [],
                dataKeysValues = {};

            //        for(var index in strKeys){
        ...
        /**
         * Wrap relative path to the server and return absolute URL to it
         *
         * @param {String} link relative path
         * @param {Object} [params={}] params that should be add to link (GET params)
         * @return {String}
         */
        this.wrapLink = function(link, params) {
            var href = (!this.isProductionServer() ? productionLocation[0] : '') + link,
                questionCharPos = href.indexOf('?'),
                paramsPrefixChar = (questionCharPos === -1 ? (questionCharPos === href.length - 1 ? '' : '?') : '&'),
                appendParams = params || {},
                paramsString;

            if(!this.isProductionServer()) {
                appendParams['token'] = stagingKey;
            }

            if(appendParams['module'] && this.isDevelopment()  && appendParams['module'] == 'Modules_Admin_Kpi_Management'){
                appendParams['module'] = 'Modules_Admin_Kpi_ManagementDev';
            }

            paramsString = Ext.Object.toQueryString(appendParams);

            return href + paramsPrefixChar + paramsString;
        };
        ...
        this.getUtcDate = function(sDate){
            var utcOffset = 2*60*60000;
            var date = new Date(sDate);
            var userOffset = date.getTimezoneOffset()*60000;
            var utcTime = new Date(date.getTime()+utcOffset+userOffset);

            return utcTime;
        };

        this.rateItRender = function(){
            (function ($) {
                $.rateit = {
                    aria: {
